...
(function ($, undefined) {
    ...
    function setAlign(target, align) {
        align = String(align);
        if (!$.array.contains(["left", "center", "right"], align.toLowerCase())) {
            return;
        }
        var t = $(target), state = $.data(target, "toolbar"),
    ...
    function setValign(target, valign) {
        ...
        if (!$.array.contains(["top", "middle", "bottom"], valign.toLowerCase())) {
            return;
        }
        var t = $(target), state = $.data(target, "toolbar"),
    ...
    function appendItemToContainer(target, container, item) {
        ...
        if ($.util.isDOM(item)) {
            ...
            // /^(?:div|span)$/i.test(cell[0].nodeName) &&

            if (/^(?:a)$/i.test(cell[0].nodeName) && !$.array.contains(["-", "—", "|"], text)) {
                if (!cell.hasClass('easyui-linkbutton')) {
                    cell.linkbutton({plain: true});
                }
            }
            ...
            if ($.array.contains(["-", "—", "|"], item)) {
                ...
                $(item).each(function () {
                    appendItemToContainer(target, container, this);
                });
            } else {
            ...
            if (itemOpts.id) {
                tItem.attr("id", itemOpts.id);
            }
            if (itemOpts.name) {
                tItem.attr("name", itemOpts.name);
            }
            if (itemOpts.cls) {
                container.addClass(itemOpts.cls);
            }
            if (itemOpts.itemCls) {
                tItem.addClass(itemOpts.itemCls);
            }
            if (itemOpts.style) {
                container.css(itemOpts.style);
            }
            if (itemOpts.itemStyle) {
                tItem.css(itemOpts.itemStyle);
            }
            if (itemOpts.width) {
                container.css("width", itemOpts.width);
            }
            if (itemOpts.align) {
                container.css("text-align", itemOpts.align);
            }
            if (itemOpts.htmlAttr) {
                tItem.attr(itemOpts.htmlAttr);
            }
    ...
    function appendItemOption(target, item) {
        ...
        if (!tr.length) {
            tr = $("<tr class='toolbar-row'></tr>").appendTo(state.wrapper);
        }
    ...
    function getItemIndex(target, item) {
        ...
        if (!tr.length || $.contains(tr[0], item[0])) {
            return ret;
        }
    ...
    function getItem(target, index) {
        ...
        if ($.isNumeric(index)) {
            if (tds.length >= index && index >= 0) {
                itemEle = tds.eq(index).find(".toolbar-item");
            }
        }
    ...
    function removeItem(target, index) {
        ...
        if (item) {
            if (item.actions && $.isFunction(item.actions.destroy)) {
                actions.destroy(item.target[0]);
            }
    ...
    function updateItem(target, param) {
        if (!param || !$.isNumeric(param.index) || !param.item) {
            return;
        }
    ...
    function setItemValue(target, param) {
        if (!param || !$.isNumeric(param.index)) {
            return;
        }
    ...
    function resizeItem(target, param) {
        if (!param || !$.isNumeric(param.index) || !param.width) {
            return;
        }
        var item = getItem(target, index);
        if (item) {
    ...
    function enableAll(target) {
        ...
        $.each(items, function (i, item) {
            ...
            var t = $(item.target[0]);
            if ($.trim(t.text()) != '-' && $.trim(t.text()) != '') {
                t.linkbutton('enable');
                if (item.handler) {
                    t.click(item.handler);
                }
            }
        });
    ...
    var itemTypes = {
        ...
        textbox: {
            ...
            init: function (container, options) {
                ...
                if (opts.value) {
                    this.setValue(box[0], opts.value);
                }
                if (opts.disabled) {
                    this.disable(box[0]);
                }
                if (opts.width) {
                    this.resize(box[0], opts.width);
                }
                return box;
            },
        ...
        checkbox: {
            ...
            init: function (container, options) {
                ...
                if (opts.checked) {
                    this.setValue(box[0], opts.checked);
                }
                if (opts.disabled) {
                    this.disable(box[0]);
                }
    ...
    $.fn.toolbar.methods = {
        ...
        getItemIndex: function (jq, item) {
            return getItemIndex(jq[0], item);
        },
        ...
        //  返回值：返回表示当前 easyui-toolbar 控件的 jQuery 链式对象。
        updateItem: function (jq, param) {
            return jq.each(function () {
                updateItem(this, param);
            });
        },
        ...
        //      value: 表示要设置的新值；
        //  返回值：返回表示当前 easyui-toolbar 控件的 jQuery 链式对象。
        setItemFocus: function (jq, index) {
            return jq.each(function () {
                setItemFocus(this, index);
            });
        },
        ...
        setItemValue: function (jq, param) {
            return jq.each(function () {
                setItemValue(this, param);
            });
        },
        ...
        getItemValue: function (jq, index) {
            return getItemValue(jq[0], index);
        },
        ...
        resizeItem: function (jq, param) {
            return jq.each(function () {
                resizeItem(this, param);
            });
        },
        ...
        disableAll: function (jq) {
            return jq.each(function () {
                disableAll(this);
            });
        },

        disabled: function (jq, text) {
            return jq.each(function () {
                disabled(this, text);
            });
        },
        enable: function (jq, text) {
            return jq.each(function () {
                enable(this, text);
            });
        },
